...
function renderColors(){
  ...
  qs("colorList").innerHTML=list.map(c=>`
    <div class="color">
      ...
      <div><b>${c.code}</b><br>${c.name[currentLang]}</div>
...
function addColor(code){
  ...
  if(c.series!==currentRecipe.series) return alert("Wrong series");
...
function cloneRecipe(r){
  ...
  qs("recipeName").value=currentRecipe.name+` v${currentRecipe.version}`;
  showTab("new");
}
...
function renderRecipes(){
  ...
  if(!recipes.length) return box.innerHTML=`<p>${t("noRecipes")}</p>`;

  box.innerHTML=recipes.map(r=>`
    <div class="color">
      <b>${r.name} v${r.version}</b><br>
      ${t(r.status)}
      <button onclick='cloneRecipe(${JSON.stringify(r)})'>📋</button>
    </div>
  `).join("");
}
